...
const autoLogPath = path.join(hooksDir, 'auto-log.js');
...
function copyScripts() {
  fs.copyFileSync(path.join(scriptsSourceDir, 'auto-log.js'), autoLogPath);
...
function ensureStopHook() {
  ...
  if (!alreadyExists) {
    stopEntries.push({ matcher: '', hooks: [{ type: 'command', command }] });
  }
...
function removeStopHook() {
  ...
  settings.hooks.Stop = settings.hooks.Stop
    .map((group) => {
      ...
      return { ...group, hooks: group.hooks.filter((hook) => !(hook && hook.type === 'command' && hook.command === command)) };
    })
...
function printStatus() {
  const installed = fs.existsSync(autoLogPath) && fs.existsSync(dailyReportPath);
  const settings = readJson(settingsPath, {});
  const command = `node "${autoLogPath}"`;
  ...
  console.log(`脚本目录: ${installed ? '已安装' : '未安装'}`);
